 ...
 * <p>
 * This implementation encapsulates all steps which are necessary to create a fully usable and populated device metadata
 * graph of a particular UPnP device. It starts with an unhydrated and typically just discovered
 * {@link org.fourthline.cling.model.meta.RemoteDevice}, the only property that has to be available is its
 * {@link org.fourthline.cling.model.meta.RemoteDeviceIdentity}.
 * </p>
 * <p>
 * This protocol implementation will then retrieve the device's XML descriptor, parse it, and retrieve and parse all
 * service descriptors until all device and service metadata has been retrieved. The fully hydrated device is then added
 * to the {@link org.fourthline.cling.registry.Registry}.
 * </p>
 * <p>
 * Any descriptor retrieval, parsing, or validation error of the metadata will abort this protocol with a warning
 * message in the log.
 * </p>
 *
...
public class RetrieveRemoteDescriptors implements Runnable {
    ...
    public void run() {
        ...
        try {
            ...
            log.warn("Descriptor retrieval failed: {}", deviceURL, ex);
        }
        finally {
    ...
    protected void describe() throws RouterException {
        ...
        try {
            ...
            log
                .warn("Device descriptor retrieval failed: {}, possibly invalid URL: ",
                    rd.getIdentity().getDescriptorURL(), ex);
            return;
        }

        if (deviceDescMsg == null) {
            log.warn("Device descriptor retrieval failed, no response: {}", rd.getIdentity().getDescriptorURL());
            return;
        }

        if (deviceDescMsg.getOperation().isFailed()) {
            log
                .warn("Device descriptor retrieval failed: {}, {}", rd.getIdentity().getDescriptorURL(),
        ...
        if (!deviceDescMsg.isContentTypeTextUDA()) {
            log
                .debug("Received device descriptor without or with invalid Content-Type: "
    ...
    protected void describe(String descriptorXML) throws RouterException {
        ...
        try {
            ...
            if (hydratedDevice == null) {
                ...
                if (notifiedStart)
                    getUpnpService()
                        .getRegistry().notifyDiscoveryFailure(describedDevice,
    ...
    protected RemoteDevice describeServices(RemoteDevice currentDevice)
        ...
        if (currentDevice.hasServices()) {
            ...
            for (RemoteService service : filteredServices) {
                ...
                if (svc != null)
                    ...
                    log.warn("Skipping invalid service '{}' of: {}", service, currentDevice);
            }
        }
        ...
        return currentDevice
            .newInstance(currentDevice.getIdentity().getUdn(), currentDevice.getVersion(), currentDevice.getType(),
                currentDevice.getDetails(), iconDupes, currentDevice.toServiceArray(describedServices),
    ...
    protected RemoteService describeService(RemoteService service)
        ...
        if (serviceDescMsg.getOperation().isFailed()) {
            log
                .warn("Service descriptor retrieval failed: {}, {}", descriptorURL,
