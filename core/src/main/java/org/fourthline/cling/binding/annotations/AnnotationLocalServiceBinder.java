...
public class AnnotationLocalServiceBinder implements LocalServiceBinder {
    ...
    private static Logger log = LoggerFactory.getLogger(AnnotationLocalServiceBinder.class);

    @Override
    public <T> LocalService<T> read(Class<?> clazz) throws LocalServiceBindingException {
        ...
        if (clazz.isAnnotationPresent(UpnpService.class)) {
            ...
            ServiceId serviceId =
                idAnnotation.namespace().equals(UDAServiceId.DEFAULT_NAMESPACE) ? new UDAServiceId(idAnnotation.value())
                    : new ServiceId(idAnnotation.namespace(), idAnnotation.value());

            ServiceType serviceType =
                typeAnnotation.namespace().equals(UDAServiceType.DEFAULT_NAMESPACE)
    ...
    @Override
    public <T> LocalService<T> read(
        Class<?> clazz, ServiceId id, ServiceType type, boolean supportsQueryStateVariables,
    ...
    public <T> LocalService<T> read(
        ...
        try {
            ...
            log.warn("Could not validate device model: ", ex);
    ...
    protected Map<StateVariable, StateVariableAccessor> readStateVariables(
        Class<?> clazz, Set<Class> stringConvertibleTypes) throws LocalServiceBindingException {
        ...
        if (clazz.isAnnotationPresent(UpnpStateVariables.class)) {
            ...
            for (UpnpStateVariable v : variables.value()) {
                ...
                if (getter != null && field != null) {
                    accessor =
                        variables.preferFields() ? new FieldStateVariableAccessor(field)
        ...
        for (Field field : Reflections.getFields(clazz, UpnpStateVariable.class)) {
            ...
            StateVariable stateVar =
                new AnnotationStateVariableBinder(svAnnotation,
    ...
    protected Map<Action, ActionExecutor> readActions(
        Class<?> clazz, Map<StateVariable, StateVariableAccessor> stateVariables, Set<Class> stringConvertibleTypes)
